...
### v1.0.3 (2026-09-16)
- **修复 (金额精度保留与无损展示)**：
  - 去除金额展示层原有的 `toFixed(1)` 进位截断，彻底解决如 `103.5h × ¥19.5/h = ¥2018.25` 在看板与汇总中被四舍五入进位显示为 `¥2018.3` 的问题。
  - 金额计算与格式化逻辑（`money`、`signed`、`moneyParts`）调整为保留原始数据有效位数，同时消除 JS 浮点二进制精度抖动。
  - 看板大字整数与小数拆分逻辑自适应原始小数位，保证整数部千分位与小数分位严密吻合。
- **Android 原生资产与版本**：
  - 同步静态资产至 `android/app/src/main/assets/www/`。
  - 版本号升级：`versionCode 4`，`versionName "1.0.3"`。
